...
1. Read `schema.md` once. Treat it as fixed for the session. Do not
   edit it.
...
Use shell reads (`cat`) to load files. Write files with standard shell
file-write operations. No special tool names — just the filesystem.
...
8. Append one row per processed source to `Intelligence/log.tsv`
   ...
   sole writer of `log.tsv` (subprocesses emit row data; the
...
**Parallelism.** Process one bucket at a time sequentially (Codex
does not spawn parallel subprocesses natively). Treat each bucket as
an isolated unit: enter the bucket, score candidate sources, write
articles + topic indexes inside that bucket only, collect row data,
then move to the next bucket. The orchestrator appends all log rows
and writes `Intelligence/index.md`, `Intelligence/log.tsv`,
...
Read-only audit. **Report findings; do not change anything.** Process
buckets sequentially; one orchestrator-written summary row.
...
- **One orchestrator per run.** Processes buckets sequentially,
  collects their reports, and is the **sole writer** of
  `Intelligence/index.md`, `Intelligence/log.tsv`,
  `Intelligence/_unsorted/`, and `Intelligence/_eval/results.tsv`.
- **Bucket scope.** When processing bucket `X`, write **only**
  inside `Intelligence/X/`. Any write outside is a hard-rule-8
